...
_PURPOSE_RULES: tuple[_PurposeRule, ...] = (
    _PurposeRule(
        name="채용 운영",
        keywords=("채용공고", "채용", "면접평가", "면접", "입사서류", "입사", "지원자", "recruit", "interview", "onboarding"),
    ),
    _PurposeRule(
        name="인력 확보",
        keywords=("채용공고", "채용", "면접평가", "면접", "입사서류", "지원자", "인력", "recruit", "hiring"),
    ),
    _PurposeRule(
        name="평가 운영",
        keywords=("성과평가", "평가가이드", "평가양식", "인사평가", "performance", "evaluation", "review"),
    ),
    _PurposeRule(
        name="보상 운영",
        keywords=("연봉인상", "연봉", "급여", "보상", "인상안", "salary", "compensation", "raise"),
    ),
    _PurposeRule(
        name="결산 관리",
    ...
    _PurposeRule(
        name="세무 신고",
        keywords=("부가세신고", "부가세", "세무", "세금", "신고", "vat", "tax"),
    ),
)
...
def _score_purpose_candidates(document_names: list[str]) -> list[PurposeCandidate]:
    ...
    for rule in _PURPOSE_RULES:
        ...
        confidence = _clamp(45 + len(evidence) * 12 + keyword_hits * 5, 0, 95)
        candidates.append(
            PurposeCandidate(
                name=rule.name,
